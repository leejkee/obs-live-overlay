...
function applySettings(data) {
  const { settings } = data;
  card.hidden = !settings.enabled || !data.running || !Object.values(settings.modules).some(Boolean);
  for (const element of document.querySelectorAll("[data-module]")) element.hidden = !settings.modules[element.dataset.module];
  details.hidden = !settings.modules.status && !settings.modules.title && !settings.modules.artistTitle;
  card.classList.toggle("cover-only", details.hidden);
  for (const [section, element] of [["title", title], ["artistTitle", artist]]) {
    const style = settings.typography[section];
    element.style.fontFamily = fontStacks[style.fontFamily] ?? fontStacks.system;
    ...
    element.style.webkitTextStroke = style.outlineEnabled ? `${style.outlineWidth}px ${style.outlineColor}` : "0px transparent";
...
async function update() {
  try {
    ...
    artist.textContent = session ? `${session.media?.artist || "未知歌手"} - ${session.media?.title || "等待歌曲信息"}` : "打开 QQ 音乐并播放歌曲";
