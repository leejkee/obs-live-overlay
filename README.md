...
在控制台中添加观众、指定当前上号用户、完成当前用户出队、修改队列标题和停止排队文案、显示提示消息，并调整各区域的字体、字号、颜色和描边。指定当前用户不会改变原有排队顺序，适合临时跳过不在场的观众。控制台支持浅色、深色主题，选择会保存在当前浏览器中。将 OBS Overlay 地址添加为 OBS 的 Browser Source；页面背景透明，可直接叠加到直播画面。
