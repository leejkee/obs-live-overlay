...
describe("Overlay Service", () => {
  ...
  it("提供控制台和 Overlay 页面", async () => {
    ...
    assert.match(overlayHtml, /id="queue-heading" class="queue-heading"/);
    assert.equal(overlayHtml.match(/class="queue-heading"/g)?.length, 1);
    ...
    assert.match(overlayStyles, /\.queue-header\s*\{[^}]*display:\s*flex/);
    assert.match(overlayStyles, /\.queue-heading\.stopped\s*\{/);
    assert.doesNotMatch(overlayStyles, /\.stopped-banner/);
